...
def transfer_files():
    """Handle file transfer with all credentials provided in form"""
    try:
        # Get all form data
        client_id = request.form.get('client_id', '').strip()
        client_secret = request.form.get('client_secret', '').strip()
        ...
        if not all([client_id, client_secret, oauth_project_id, drive_folder_id, 
                   gcs_bucket_name, gcs_project_id, service_account_key]):
            flash('יש למלא את כל השדות הנדרשים', 'error')
            return redirect(url_for('index'))

        # Store in session and redirect to OAuth
        session['client_id'] = client_id
        session['client_secret'] = client_secret
        session['oauth_project_id'] = oauth_project_id
        session['gcs_project_id'] = gcs_project_id
        session['gcs_bucket_name'] = gcs_bucket_name
        session['service_account_key'] = service_account_key
